...
export interface components {
  schemas: {
    ...
    BaseOrder: {
      ...
      parking_time?: number;
      /**
       * @description The priority level of this order to get assigned in whole optimization. E.g, orders with low order priority (bigger number) will not get assigned when there's not enough vehicles.
       *
       * @default 3
       */
      assignment_priority?: number;
      /** @description Prioritize the visiting sequence of this order in a route. E.g, orders with high sequence priority (smaller number) will be visited earlier than orders that with low priority (bigger number) in the same route. Default is `null`, which means no priority relationship is specified. */
    ...
    OrderVrp: {
      ...
      mx_label?: string;
      /**
       * @description If false, cost per order at this location will be ignored (at least 1 will be counted).
       *
       * @default true
       */
      is_unique?: boolean;
    ...
    CostParamsVrp: {
      ...
      cost_per_unit_quantity?: number | number[];
      /**
    ...
    VehicleType: {
      ...
      /**
       * @description If solver dispatchs this vehicle type, setting this value to `true` will dispatch all available vehicles.
       *
       * @default false
       */
      use_all_vehicles?: boolean;
      /** @description Start_depot id. Set to `any` for start depot automation, set to `none` to start from any order. */
      start_depot?: string | unknown[];
      /** @description End_depot id. Set to `any` for end depot automation, set to `none` for open end routing. */
      end_depot?: string | unknown[];
    ...
    SolverParamsVrp: {
      ...
      /** @description Specify the time point in ISO 8601 format for traffic query. For example, `2021-01-01T14:00:00-08:00` means 2021 Jan 1st, 2PM, UTC-08:00. If not specified, current time is used. Not available with OpenStreetMap. */
      traffic_time?: string;
    ...
    ViolationDetails: [number, string, string][];
    RoutingResponse: {
      /** @enum {string} */
      status?: "SUCCEED";
